...
На карточке аккаунта всегда видна кнопка **Получить новый QR WhatsApp**. Она требует браузерного подтверждения; для отключённого аккаунта кнопка одновременно включает его. Команда сохраняется в БД, worker вызывает настоящий WhatsApp logout, удаляет только auth-каталог выбранного аккаунта и после подтверждения готовит свежий QR. Сообщения, email, пароль и TOTP при этом сохраняются.
