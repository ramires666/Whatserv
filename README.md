...
2. Затем настройте TOTP: можно вставить обычный Base32-секрет или целую ссылку `otpauth://totp/...`. Дополнительный шестизначный код не требуется. TOTP ожидается по умолчанию; для редкого аккаунта без него нужно явно отметить соответствующую галочку.
3. Телефон необязателен и сохраняется как произвольная подпись без проверки формата. Уникальным логином аккаунта остаётся email; один телефон можно указать у нескольких аккаунтов.
4. Нажмите **Сохранить аккаунт**. Это не запускает WhatsApp автоматически. Если телефон уже введён, можно вместо этого явно нажать **Сохранить и войти через QR WhatsApp**.
5. Сохраните показанную индивидуальную ссылку. Позднее её можно снова показать или скопировать на карточке аккаунта.
6. Если WhatsApp ещё не запускался, нажмите на карточке **Войти через QR WhatsApp**. Кнопка появляется только после сохранения непустого телефона. В течение примерно 15 секунд на карточке появится QR.
7. На телефоне откройте WhatsApp → **Связанные устройства** → **Привязка устройства** и отсканируйте QR.
8. Статус изменится на `online`, QR автоматически исчезнет. При истечении QR шлюз выдаст свежий; после выхода связанного устройства будет запрошена новая привязка.

Главная страница админки по умолчанию показывает компактный свёрнутый список аккаунтов. Нажатие на строку аккаунта раскрывает его QR, TOTP, последнее сообщение, ссылку и настройки. Полная форма создания скрыта под верхней кнопкой **Ввести аккаунт** и автоматически раскрывается повторно, если сервер нашёл ошибку во введённых данных.

TOTP-секрет проверяется по формату перед сохранением и хранится только в зашифрованном виде. В админке виден лишь статус `TOTP настроен` / `без TOTP`; исходный секрет никогда не показывается повторно. При необходимости TOTP можно добавить, заменить или удалить на карточке аккаунта без ввода текущего кода.
...
```text
https://example.com/inbox/<uuid-аккаунта>/<случайный-секрет>
```
...
У подключённого аккаунта с заполненным телефоном видна кнопка **Получить новый QR WhatsApp**. Она требует браузерного подтверждения. Команда сохраняется в БД, worker вызывает настоящий WhatsApp logout, удаляет только auth-каталог выбранного аккаунта и после подтверждения готовит свежий QR. Сообщения, email, пароль и TOTP при этом сохраняются.
