...
Страница показывает email, пароль по отдельной кнопке (автоматически скрывается через 30 секунд), состояние WhatsApp, последние входящие сообщения и текущий TOTP-код с кнопкой копирования и обратным отсчётом. Она обновляется каждые три секунды. По умолчанию ссылка действует 720 часов; срок задаётся `CAPABILITY_TTL_HOURS`. В админке уникальная ссылка видна прямо в карточке, копируется отдельной кнопкой, а клик по карточке открывает её в новой вкладке. Кнопка **Новая ссылка** немедленно отзывает старую.
