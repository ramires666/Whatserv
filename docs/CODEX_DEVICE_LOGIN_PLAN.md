...
```mermaid
flowchart TD
    admin["Администратор WhatServ"]
    control["WhatServ control-plane"]
    agent["whatserv-codex-agent<br/>на целевой машине"]
    appserver["Локальный codex app-server"]
    login["account/login/start<br/>chatgptDeviceCode"]
    credentials["Credentials сохраняются<br/>только локально"]
    result["В WhatServ возвращаются<br/>только code / URL / status"]

    admin -->|"start login(target_id)"| control
    control -->|"Исходящий канал агента"| agent
    agent -->|"stdio JSON-RPC"| appserver
    appserver --> login
...
stateDiagram-v2
    ...
    queued --> claimed
    claimed --> starting
    starting --> waiting_for_user
    waiting_for_user --> authenticated
